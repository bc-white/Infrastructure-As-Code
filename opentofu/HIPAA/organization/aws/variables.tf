...
variable "github_org" {
  description = "GitHub organization or user name for OIDC trust relationship"
  type        = string
  validation {
    condition     = can(regex("^[a-zA-Z0-9-]+$", var.github_org))
    error_message = "GitHub org must contain only letters, numbers, and hyphens."
  }
  validation {
    condition     = length(var.github_org) > 0
    error_message = "GitHub org must not be empty."
  }
}
