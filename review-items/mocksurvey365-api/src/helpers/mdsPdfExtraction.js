...
function parsePDFText(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    // Look for the data section start - lines with resident names
    if (lines[i].match(/^[A-Z]+,\s*[A-Z]+\d{8,10}\d{2}\/\d{2}\/\d{2}/)) {
      headerLineIndex = i - 1; // Start processing from this line
      console.log("Found data section starting at line:", i);
      console.log("First data line:", lines[i]);
      break;
    }
  }
  
  if (headerLineIndex === -1) {
    console.log("Could not find data section, trying to find resident lines directly...");
    ...
    // Parse table data starting from the data section
    for (let i = headerLineIndex; i < lines.length; i++) {
      ...
      // Only process lines that match resident pattern
      if (line.match(/^[A-Z]+,\s*[A-Z]+\d{8,10}\d{2}\/\d{2}\/\d{2}/)) {
        const resident = parseTableRow(line);
        if (resident && resident.name) {
          residents.push(resident);
