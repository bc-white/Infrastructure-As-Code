...
const Tabs = ({
  ...
  return (
    <nav
      ref={navRef}
      className="flex items-center gap-1 relative z-0"
      ...
      {tabs.map((item, i) => {
        ...
        return (
          <button
            key={item.value}
            className={cn(
              "text-sm relative rounded-md flex items-center h-9 px-3 z-20 cursor-pointer select-none transition-colors border",
              isActive 
...
export function AnimatedTabs({ tabs, onTabChange, showContent = true, children }: AnimatedTabsProps) {
  ...
  }, [framer.selectedTab.value, onTabChange]);

  return (
    <div className="w-full">
      <div className="relative flex w-full items-center overflow-x-auto overflow-y-hidden">
